...
namespace Loops
{
    class Program
    {
        static void NotMain(string[] args)
        {
            ...
            /*while (!isGuessed) ;*/ /*Shorthand for "isGuessed == false". Be sure to assign a var 
            first. The flaw with this while loop is; what if you guessed the number correctly the
            first time? The while loop never hits, so nothing happens when you run the 
            program.*/

            for (int i = 1; i <= 96; i++) //This for loop displays only even numbers up to 96
            {
                if (i % 2 == 0)
                {
                    Console.Write(i + ", ");
